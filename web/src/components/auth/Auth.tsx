...
export default function Auth({ setIsLoggedIn }: IAuthProps) {
  ...
  let attemptLogin = async (
    ...
    e.preventDefault();
    axios
      .post(`${API_ROUTE}/api/auth/${authMethod}`, {
        username,
        password,
      })
      .then((response) => {
  ...
  let switchTag =
    authMethod == "signup" ? (
      <p className="switchtag">
        Already have an account?{" "}
        <button onClick={(e) => switchAuthMethod(e)} className="switch">
          Log In
        </button>
      </p>
    ) : (
    ...
    if (serverError) {
      return (
        <p className="error-text">
          {serverError.message.charAt(0) + serverError.message.slice(1) + "."}
        </p>
      );
    } else {
      return null;
  ...
  return (
    <div id="Auth">
      <form>
        <p className="header-1">welcome to disrupted.</p>
        <p className="header-2">join us in the future of connectivity.</p>
        ...
        <Input
          ...
          update={setUsername}
          errors={
            validationErrors.filter((err) => err.property === "username")[0]
          }
        />
        ...
        <Input
          ...
          update={setPassword}
          errors={
            validationErrors.filter((err) => err.property === "password")[0]
          }
        />
