...
export default function Input({element, password, update, errors}: IInputProps) {
    ...
    let errorMap = () => {
        if(errors) {
            return Object.values(errors.constraints).map(a => <p className="error-text">{a.charAt(0).toUpperCase() + a.slice(1) + '.'}</p>)
        } else {
            return null
        }
    }
